<?php
    ...
    $device_status = isset($_POST['status']) ? $_POST['status'] : null;
	...
	if($powerloss_db =="YES")
	{
		$query_update = "UPDATE devices SET return_status=?, _datetime = ?, Powerloss=? WHERE device = ?";// dodat i da checka user id
		...
		$ok = mysqli_stmt_bind_param($stmt2, "ssss", $device_status, $date, $Powerloss, $device_name);//doadt i user id
		if ($ok === false) {
		error_log(mysqli_stmt_error($db));
		die("Sorry, there has been a software error");
		}
...
'X-Mailer: PHP/' . phpversion();
	...
	{
		$query_update = "UPDATE devices SET return_status=?, _datetime=? WHERE device = ?";// dodat i da checka user id

		$stmt1 = mysqli_prepare($db, $query_update);
		if ($stmt1 === false) {
		error_log(mysqli_error($db));
		die("Sorry, there has been a software error");
		}
		$ok = mysqli_stmt_bind_param($stmt1, "sss",$device_status, $date ,$device_name);//doadt i user id
		if ($ok === false) {
		error_log(mysqli_stmt_error($db));
		die("Sorry, there has been a software error");
		}
